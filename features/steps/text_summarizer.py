...
@when(u'I submit 50 for meal cost and 20 for tip percentage')
def step_impl(context):
    br = context.browser
    br.get('http://localhost:5000')
    meal_cost = br.find_element_by_name("meal_cost")
...
@then(u'I should see 10 displayed on the results page')
def step_impl(context):
    br = context.browser
    result = br.find_element_by_id('results').text
    assert result == "10.0"

@when(u'I submit 30 for meal cost and -15 for tip percentage')
def step_impl(context):
    br = context.browser
    br.get('http://localhost:5000')
    meal_cost = br.find_element_by_name("meal_cost")
    meal_cost.send_keys("30")
    tip_percentage = br.find_element_by_name("tip_percentage")
    tip_percentage.send_keys("-15")
    br.find_element_by_id("submit").click()

@then(u'I should see 4.5 displayed on the results page')
def step_impl(context):
    br = context.browser
    result = br.find_element_by_id('results').text
    assert result == "4.5"    